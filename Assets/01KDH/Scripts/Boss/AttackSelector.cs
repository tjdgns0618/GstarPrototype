...
public class AttackSelector : INode
{
    ...
    public INode.ENodeState Evaluate()
    {
        ...
        if(distance <= _boss.__attackRange)
        {
            return _boss.DoNormalAttack();
        }

        switch (_boss._bossType)
        {
            case BossType.Worm:
                if (distance < (_boss._attackRange * _boss._attackRange))                                                   // 공격 사거리 내에 들어왔을 경우
                ...
                if (distance <= (_boss._patternRange * _boss._patternRange) && _boss._isSecondOn)          // 패턴 사거리보다 가까우면서 두 번째 패턴의 쿨타임이 아닐 때
                    return _boss.DoFirstPattern();                                                                                          // 두 번째 패턴 공격 (도망가기)
                if (distance >= (_boss._attackRange * _boss._attackRange))                                                                                                                                            // 사거리 밖일 경우
